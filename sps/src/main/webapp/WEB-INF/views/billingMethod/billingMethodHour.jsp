...
<!-- 消费类型修改的模态框 -->
	    ...
	    <div class="modal-content">
	      ...
	      <div class="modal-body">
	        ...
	        <div class="form-group">
			    ...
			    <div class="col-sm-10">
			      <input type="text" name="billingMethodHourFreeMinutes" class="form-control" id="billingMethodHourFreeMinutes_update_input" placeholder="请输入要修改的免费分钟（分）">
			  ...
			  <div class="form-group">
			    ...
			    <div class="col-sm-10">
			      <input type="text" name="billingMethodHourStartPrice" class="form-control" id="billingMethodHourStartPrice_update_input" placeholder="请输入要修改的起步费用（元）">
				  <span class="help-block"></span>
			    </div>
			  </div>
			  <div class="form-group">
			    <label class="col-sm-2 control-label">每日最高消费（元）</label>
			    <div class="col-sm-10">
			      <input type="text" name="billingMethodHourDayPrice" class="form-control" id="billingMethodHourDayPrice_update_input" placeholder="请输入要修改的每日最高消费（元）">
			      <span class="help-block"></span>
			    </div>
			  </div>
			   <div class="form-group">
			    <label class="col-sm-2 control-label">每小时费用（元）</label>
			    <div class="col-sm-10">
			      <input type="text" name="billingMethodHourAddPrice" class="form-control" id="billingMethodHourAddPrice_update_input" placeholder="请输入要修改的每小时费用（元）">
			      <span class="help-block"></span>
			    </div>
			  </div>
			</form>
	...
	<script type="text/javascript">
		...
		function to_page(pn){
			$.ajax({
				url:"${APP_PATH}/getBillingMethodHour",
				data:{"pn":pn},
				type:"GET",
				cache:false,
		...
		function build_roles_table(result){
			...
			var methodHour = result.object;
			var checkBoxTd = $("<td  style='text-align:center;vertical-align:middle;'><input type='checkbox' class='check_item'/></td>");
			var billingMethodHourId = $("<td style='display: none;text-align:center;vertical-align:middle;'></td>").append(methodHour.billingMethodHourId);
			var billingMethodHourFreeMinutes = $("<td style='text-align:center;vertical-align:middle;'></td>").append(methodHour.billingMethodHourFreeMinutes);
			var billingMethodHourStartPrice = $("<td style='text-align:center;vertical-align:middle;'></td>").append(methodHour.billingMethodHourStartPrice);
			var billingMethodHourDayPrice = $("<td style='text-align:center;vertical-align:middle;'></td>").append(methodHour.billingMethodHourDayPrice);
			var billingMethodHourAddPrice = $("<td style='text-align:center;vertical-align:middle;'></td>").append(methodHour.billingMethodHourAddPrice);
			
			var editBtn = $("<button></button>").addClass("btn btn-primary btn-sm edit_btn")
							.append($("<span></span>").addClass("glyphicon glyphicon-pencil")).append("编辑");
			//为编辑按钮添加一个自定义的属性，来表示当前角色id
			editBtn.attr("edit-id",methodHour.billingMethodHourId);
			var btnTd = $("<td style='text-align:center;vertical-align:middle;'></td>").append(editBtn);
			//var delBtn = 
			//append方法执行完成以后还是返回原来的元素
			$("<tr></tr>")
				.append(billingMethodHourId)
				.append(billingMethodHourFreeMinutes)
				.append(billingMethodHourStartPrice)
				.append(billingMethodHourDayPrice)
				.append(billingMethodHourAddPrice)
				.append(btnTd)
		...
		function getBillingMethod(){
			$.ajax({
				url:"${APP_PATH}/getBillingMethodHourById",
				type:"GET",
				cache:false,
				success:function(result){
					var billingMethod = result.extend.billingMethod;
					$("#billingMethodHourFreeMinutes_update_input").val(billingMethod.billingMethodHourFreeMinutes);
					$("#billingMethodHourStartPrice_update_input").val(billingMethod.billingMethodHourStartPrice);
					$("#billingMethodHourDayPrice_update_input").val(billingMethod.billingMethodHourDayPrice);
					$("#billingMethodHourAddPrice_update_input").val(billingMethod.billingMethodHourAddPrice);
				}
			});
		}
		//校验起步费用（元）是否可用
		...
		$("#billingMethod_update_btn").click(function(){
			//2、发送ajax请求保存更新的角色数据
			$.ajax({
				url:"${APP_PATH}/updateBillingMethodHour",
				type:"PUT",
