...
$(function(){
	...
	$.ajax({
		...
		success:function(result){
			...
			var menu = [];
			var rootNode  ={};
			var rolemenu = result.extend.roleMenuList;
			rootNode.id ='1';
			rootNode.name ='主菜单';
			rootNode.parentId ='0';
			rootNode.url ='';
	...
	function getMenus(rolemenu,parentMenuName,parentNode){
		for(var i =0;i<rolemenu.length;i++){
			var menu = rolemenu[i];
			if(menu.roleMenuMenu.menuSuperior ==parentMenuName){
				var secondNode  = {};
				secondNode.id =menu.roleMenuMenu.menuId;
				secondNode.name =menu.roleMenuMenu.menuName;
				secondNode.parentId =parentNode.id;
				if(menu.roleMenuMenu.menuUrl == null){
					secondNode.url ="";
				}else{
					secondNode.url =menu.roleMenuMenu.menuUrl;
				}
				secondNode.icon ='';
				secondNode.order ='1';
				secondNode.isHeader ='0';
				secondNode.childMenus=[];
				getMenus(rolemenu,menu.roleMenuMenu.menuName,secondNode);
