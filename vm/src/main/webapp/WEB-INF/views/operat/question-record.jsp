...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 运营管理 <span class="c-gray en">&gt;</span> 跟办问题 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l">
		<a href="javascript:;" onclick="datadel()" class="btn btn-danger radius">
			<i class="Hui-iconfont">&#xe6e2;</i> 批量删除
		</a> 
		<a href="javascript:;" onclick="admin_add('添加计划','/web/PavePlanAdd','500','400')" class="btn btn-primary radius">
	...
	<table id="admin_table" class="table table-border table-bordered table-bg">
		<thead>
			<tr>
				<th scope="col" colspan="9">计划列表</th>
			</tr>
			<tr class="text-c">
				<!-- <th style="display: none;width: 100px;">主键</th> -->
				<th width="25"><input type="checkbox" name="" value=""></th>
				<th>问题类型</th>
				<th style="width: 400px;">问题内容</th>
				<th>渠道名称</th>
				<th>点位名称</th>
				<th>运营人员</th>
				<th>问题状态</th>
	...
	*/
	function build_roles_table(result){
		...
		$.each(pglist,function(index,item){
				...
				var pavePlanId = $("<td style='display: none;'></td>").append(item.pavePlanId);
				var pavePlanTitle = $("<td></td>").append(item.pavePlanTitle).css("text-align","center");
				var pavePlanComit = $("<td></td>").append(item.pavePlanComit).css("text-align","center");
				var pavePlanTime = item.pavePlanTime;
				if (pavePlanTime == null) {
					pavePlanTime = $("<td></td>").append("-").css("text-align","center");
				}else{
					pavePlanTime = $("<td></td>").append(item.pavePlanTime).css("text-align","center");
				}
				
				var pavePlanAdminId = item.admin;
				if (pavePlanAdminId != null) {
					pavePlanAdminId = $("<td></td>").append(item.admin.adminRealname).css("text-align","center");
				}else{
					pavePlanAdminId = $("<td></td>").append("-").css("text-align","center");
				}
				var pavePlanStatus = item.pavePlanStatus;
				
				var editBtn = $("<button></button>").addClass("btn btn-primary btn-sm edit_btn")
								.append($("<span></span>").addClass("glyphicon")).append("编辑");
				editBtn.attr("edit-id",item.pavePlanId);
				var delBtn =  $("<button></button>").addClass("btn btn-danger btn-sm delete_btn")
								.append($("<span></span>").addClass("glyphicon")).append("删除");
				delBtn.attr("del-id",item.pavePlanId);
				var btnTd = $("<td></td>").append(editBtn).append(" ").append(delBtn).append(" ").css("width","100px");
				
				$("<tr></tr>").append(checkBoxTd)
					.append(pavePlanId)
					.append(pavePlanTitle)
					.append(pavePlanComit)
					.append(pavePlanTime)
					.append(pavePlanAdminId)
					.append(btnTd)
