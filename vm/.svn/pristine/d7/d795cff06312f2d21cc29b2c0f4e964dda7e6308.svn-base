...
<div class="page-container">
	<div class="text-c"> <!-- 日期范围：
		...
		<input type="text" class="input-text" style="width:250px" placeholder="输入商户名称" id="content" name="">
		<button type="button" class="btn btn-success" id="select_commer" name=""><i class="Hui-iconfont">&#xe665;</i> 搜商户</button>
	</div>
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l">
		...
		<a href="javascript:;" onclick="comm_add('添加商户','/web/addCommer','800','650')" class="btn btn-primary radius">
...
var totalRecord,currentPage;
		...
		*/
		function to_page(pn){
			...
			$.ajax({
				url:"${APP_PATH}/getCommercialtAll",
		...
		function build_roles_table(result){
			...
			$.each(menu,function(index,item){
				var checkBoxTd = $("<td><input type='checkbox' value='"+item.commercialTenantId+"' name='commercialTenantId' class='check_item'/></td>").css("padding-left","14px");
				var commercialTenantId = $("<td style='display: none;'></td>").append(item.commercialTenantId);
				var commercialTenantName = $("<td></td>").append(item.commercialTenantName).css("text-align","center");
				var commercialTenantPerson = $("<td></td>").append(item.commercialTenantPerson).css("text-align","center");
				var commercialTenantDescribe = $("<td></td>").append(item.commercialTenantDescribe).css("text-align","center");
				var commercialTenantAttr = $("<td></td>").append(item.commercialTenantAttr).css("text-align","center");
				
				var commercialTenantPayType = $("<td></td>").css("text-align","center");
				if (commercialTenantId != null) {//根据渠道id查找对应的支付方式
					$.ajax({
						url:"${APP_PATH}/getPaymentByCommerId",
						data:{"commerId":item.commercialTenantId},
						type:"POST",
						success:function(result){
							...
							if (payment != "") {
								$.each(payment,function(index,item){
									$.each(item.payments,function(index,p){
										...
										commercialTenantPayType.append(to);
									})
								})
							}else{
								commercialTenantPayType.append("");
							}
				...
				var commercialTenantPhone = $("<td></td>").append(item.commercialTenantPhone).css("text-align","center");
				var commercialTenantLogo = $("<td></td>").append("<img src='"+item.commercialTenantLogo+"' width='50' height='50' >").css("text-align","center");
				var commercialTenantOfficialWebsite = $("<td></td>").append(item.commercialTenantOfficialWebsite).css("text-align","center");
				...
				editBtn.attr("edit-id",item.commercialTenantId).attr("onclick","commer_edit('商品编辑','/web/EditCommer?id="+item.commercialTenantId+"',"+item.commercialTenantId+",'800','550')");
				...
				delBtn.attr("del-id",item.commercialTenantId);
				var btnTd = $("<td></td>").append(editBtn).append(" ").append(delBtn).append(" ").css("width","130px");
				//var delBtn = 
				//append方法执行完成以后还是返回原来的元素
				$("<tr></tr>").append(checkBoxTd)
					.append(commercialTenantId)
					.append(commercialTenantName)
					.append(commercialTenantDescribe)
					.append(commercialTenantPerson)
					.append(commercialTenantAttr)
					.append(commercialTenantPhone)
					.append(commercialTenantLogo)
					.append(commercialTenantOfficialWebsite)
					.append(btnTd)
...
$(document).on("click",".delete_btn",function(){
	...
	var CommercialtName = $(this).parents("tr").find("td:eq(2)").text();
	var id = $(this).attr("del-id");
	layer.confirm('确认要删除"'+CommercialtName+'"吗？',function(index){
			//确认，发送ajax请求删除即可
			$.ajax({
				url:"${APP_PATH}/deleteCommercialt",
				type:"POST",
				data:{"CommercialtId":id,"CommercialtIds":null},
				success:function(result){
...
function datadel(){
	...
	var checkBoxAll = $("input[name='commercialTenantId']:checked");
	var ids = [];
	...
	//alert(ids);
		$.ajax({
			url:"${APP_PATH}/deleteCommercialt",
			type:"POST",
			data:{"CommercialtId":0,"CommercialtIds":ids},
			traditional :true,
			success:function(result){
				var status = result.code;
				if(status == 100){
...
/*商户-增加*/
function comm_add(title,url,w,h){
	layer_show(title,url,w,h);
}
	
/*商户-编辑*/
function commer_edit(title,url,id,w,h){
