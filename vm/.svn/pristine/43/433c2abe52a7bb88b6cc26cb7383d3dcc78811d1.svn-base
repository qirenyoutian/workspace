...
<article class="page-container">
	...
	<div class="row cl" style="display:none;">
		...
		<div class="formControls col-xs-8 col-sm-9">
			<input type="text" class="input-text" value="${param.equipmentId}" placeholder="" id="equipmentId" name="equipmentId">
		</div>
	</div>
	<div class="row cl">
	...
	<div class="row cl" id="channelMerchandiseDIV">
		...
		<div class="formControls col-xs-8 col-sm-9" style="width:250px;">
			<select id="channelMerchandiseId"  name="channelMerchandiseId" class="input-text"></select>
		</div>
	</div>
	<div class="row cl">
...
//选择渠道商品名称
	...
	$("#save_Aisle").click(function(){
		...
		//alert(aisleId+aisleNumber+merchandiseId+aisleInventory);
		  $.ajax({
			url:"${APP_PATH}/saveAisle",
			type:'POST',
			data:{"aisleId":aisleId,"merchandiseNum":merchandiseId,"aisleNumber":aisleNumber,"aisleInventory":aisleInventory,"channelMerchandiseId":channelMerchandiseId},
			async:false,
			success:function(result){
				var a = result.code;
				if (a == 100) {
					...
					layer.msg('上架失败!',{icon:5,time:3000});
				} 
			}
		}); 
